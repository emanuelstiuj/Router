...
struct trie *create_node_trie()
{
	struct trie *node = malloc(sizeof(struct trie));
	node->next_bit[0] = NULL;
	node->next_bit[1] = NULL;
	node->has_next_hop = 0;
	node->next_hop = 0;
	node->interface = -1;
...
struct trie *create_trie(struct route_table_entry *rtable, int rtable_len)
{
	struct trie *trie = create_node_trie();
	...
	for (int i = 0; i < rtable_len; i++) {
		...
		uint32_t ip_prefix = ntohl(rtable[i].mask) & ntohl(rtable[i].prefix);

		struct trie *node = trie;
		int curr_bit;

		while (prefix_len) {
			if (ip_prefix & 0x80000000)
				curr_bit = 1;
			else
				curr_bit = 0;

			if (node->next_bit[curr_bit] == NULL)
				node->next_bit[curr_bit] = create_node_trie();

			if (prefix_len == 1) {
				node->next_bit[curr_bit]->has_next_hop = 1;
				node->next_bit[curr_bit]->next_hop = rtable[i].next_hop;
				node->next_bit[curr_bit]->interface = rtable[i].interface;
				break;
			} else {
				node = node->next_bit[curr_bit];
				prefix_len--;
...
struct trie *get_best_route_trie(uint32_t ip_dest, struct trie *trie)
{	
	struct trie *node = trie;
	struct trie *best = NULL;

	uint32_t ip_addr = ntohl(ip_dest);
	int curr_bit;
	...
	while (1) {
		...
		if (node->next_bit[curr_bit]->has_next_hop)
				best = node->next_bit[curr_bit];
